...
base_values AS (
    SELECT
        ...
        MAX(CASE WHEN year = {{ var('base_year', 2003) }} THEN income_real_cpi_2023 END) AS income_cpi_2023_base,
        MAX(CASE WHEN year = {{ var('base_year', 2003) }} THEN healthcare_pc_cpi_2023 END) AS healthcare_cpi_2023_base,
        MAX(CASE WHEN year = {{ var('base_year', 2003) }} THEN childcare_annual_cpi_2023 END) AS childcare_cpi_2023_base
    FROM wide_values
    GROUP BY geo_id
)
SELECT

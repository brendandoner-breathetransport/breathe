...
### Limitations, Disputed Evidence & Areas Needing Further Research

The following flags claims in this analysis where evidence is weak, where independent scrutiny would likely push back, or where deeper research is needed before strong conclusions can be drawn. See `docs/co_ballot_2025_analysis.md` for full discussion of each.

* **State attendance trends are descriptive, not causal** — No study has used a control group or difference-in-differences design to isolate the meal effect from COVID recovery, attendance campaigns, and other concurrent interventions in the 2022–23 programs. Needed: quasi-experimental studies with school- or district-level control groups within the same states.

* **The $260K dropout cost figure is methodologically disputed** — Belfield & Levin (2007) and Alliance for Excellent Education aggregate outcomes without fully controlling for pre-existing disadvantage (selection bias). Actual causal effect of education on social costs is likely lower. Needed: instrumental variable estimates of the full social cost of dropout status independent of confounding factors.

* **CEP and breakfast program research may not generalize to statewide universal programs** — Effect sizes from older programs are concentrated in high-poverty, low-participation districts (Ruffini 2022). States with already-high meal uptake (like Colorado) may see smaller marginal effects. Needed: program-specific evaluations using student-level longitudinal data; California's UC ANR study is the most promising near-term source.

* **Cognitive tax of scarcity finding (Mani et al. 2013) has replication concerns** — The "13 IQ point" bandwidth cost of financial stress was not reproduced under a 2021 pre-registered replication. The directional claim (stress impairs cognition) is supported by stress neuroscience broadly, but the specific magnitude is contested. Needed: well-powered pre-registered replication with diverse real-world samples.

* **The mental health causal chain is established in parts, not end-to-end** — No single study traces universal meals → food insecurity reduction → anxiety/depression reduction → academic improvement in one connected design. The chain is plausible and directionally consistent, but it is constructed from separately-sourced links. Needed: longitudinal studies with validated clinical mental health instruments (e.g., PHQ-A, GAD-7) tracking the same students before and after program launch.

* **The U.S. competitiveness argument requires too many inferential leaps for direct attribution** — Hanushek & Woessmann operate at the national level over decades; the 0.06–0.10 SD effect from meal programs is small relative to the U.S. international achievement gap (~1.0–1.5 SD). Competitiveness framing is appropriate as context for educational investment broadly, not as a specific attributable outcome of this program. Should be presented as motivation, not as a demonstrated causal link.

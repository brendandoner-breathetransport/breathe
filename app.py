...
sources = {
    "economy": get_source(
        name="Sources",
        link="https://github.com/brendandoner-breathetransport/breathe/wiki/Economy",
    ),
    "economy_f150": get_source(
        name="Sources",
        link="https://github.com/brendandoner-breathetransport/breathe/wiki/Economy#ford-f-seriesf-150-historical-msrp-1950-2025",
    ),
    "economy_house_purchase_price": get_source(
        name="Sources",
        link="https://github.com/brendandoner-breathetransport/breathe/wiki/Economy#housing",
    ),
    "wid": get_source(
        name="Source: World Inequality Database", link="https://wid.world/wid-world/"
    ),
    "irs": get_source(
        name="Source: Internal Revenue Service",
        link="https://www.irs.gov/statistics/soi-tax-stats-historical-table-23",
    ),
    "1619": get_source(
        name="Source: The 1619 Project",
        link="https://www.nytimes.com/interactive/2019/08/14/magazine/1619-america-slavery.html",
    ),
    "american_character": get_source(
        name="Source: American Character",
        link="https://colinwoodard.com/books/american-character/",
    ),
    "american_dream": get_source(
        name="Sources",
        link="https://github.com/brendandoner-breathetransport/breathe/wiki/American-Dream",
    ),
    "healthcare": get_source(
        name="Sources",
        link="https://github.com/brendandoner-breathetransport/breathe/wiki/Healthcare",
    ),
}
...
def plot_stick_figure(fig, x, y, add_hat=False):
    ...
    if add_hat:
        fig.add_shape(
            ...
            y0=(y1 + 80000),
            y1=(y1 + 80000),
            line=dict(color=color_legs, width=2),
            fillcolor=color_legs,
        )
        fig.add_shape(
            # hat
            name="none",
...
def get_income_mean(group, data):
    income_mean = (
        data.filter(pl.col("group") == group).select("value").to_numpy().flatten()[0]
    )
    return income_mean


def plot_timeseries_multiple_countries(
    data, title, yaxis_title, xaxis_title, dark_mode
):
    data = data.filter(pl.col("year") >= 2000)
    data_most_recent = data.join(
        other=(data.group_by("country").agg(pl.max("year"))),
        on=["country", "year"],
        how="inner",
    )
        ...
        data=(
            [
                go.Scatter(
                    ...
                    else dict(color="rgba(0,0,0,0.2)", width=2),
                )
            ...
            + [
                go.Scatter(
                    ...
                    text=f"<b>{country.title()}</b>"
                    if country == "united states"
                    else f"<br>{country.title()}"
                    if country == "canada"
                    else f"{country.title()}",
...
def get_highlights_line_min_max(data, col_date, col_metric, number_type, max_or_min):
    ...
    if (
        data_latest[col_metric].to_numpy().flatten()[0]
        != data_max[col_metric].to_numpy().flatten()[0]
    ) & (max_or_min in ["max", "both"]):
        highlights = highlights + highlight_max
    # make sure the most recent value (latest) is not equal to the min
    if (
        data_latest[col_metric].to_numpy().flatten()[0]
        != data_min[col_metric].to_numpy().flatten()[0]
...
config["plotly_mobile"] = {
    ...
    "modeBarButtonsToRemove": [
        ...
        "zoom2d",
        "zoomIn2d",
        "zoomOut2d",  # Remove zoom buttons
        "autoScale2d",
        "resetScale2d",  # Remove scale buttons
    ],
    "toImageButtonOptions": {
        "format": "png",
...
def plot_county_heatmap(
    ...
    data_filtered = (
        data.filter(pl.col("metric") == metric)
    ...
    ui.page_navbar(
        ...
        ui.nav_panel(
            ...
            ui.row(
                ui.h1(
                    ui.span(
                        HTML("How healthy is the economy for Americans?"),
                        style="color:rgba(255,255,255,0.9)",
                    )
                )
            ),
            ...
            ui.row(
                ui.layout_columns(
                    ui.input_radio_buttons(
                        ...
                        choices={
                            "Bottom 50%": ui.span(
                                "Bottom 50%", style="color:rgba(255,255,255,0.6)"
                            ),
                            "Upper 51-99%": ui.span(
                                "Upper 51-99%", style="color:rgba(255,255,255,0.6)"
                            ),
                            "Top 1%": ui.span(
                                "Top 1%", style="color:rgba(255,255,255,0.6)"
                            ),
                ...
                ui.layout_columns(
                    ui.input_radio_buttons(
                        ...
                        choices={
                            ...
                            for country in shares_wid.select(["country"])
                            .unique()
                            .sort(["country"])
                            .to_numpy()
                            .flatten()
            ...
            ui.row(
                ui.layout_columns(
                    ui.card(
                        output_widget(
                            "plot_economy_house_purchase_cost_as_percent_of_income"
                        )
                    ),
            ...
            ui.row(
                ui.layout_columns(
                    ...
                    ui.card(
                        output_widget("plot_american_dream_mobility_international")
                    ),
                    # Stack on mobile, side-by-side on desktop
                    col_widths={"xs": (12, 12), "sm": (12, 12), "md": (6, 6)},
                )
        ...
        ui.nav_panel(
            ...
            ui.row(
                ui.h1(
                    ui.span(
                        HTML(
                            "What is the cost and performance of the American healthcare system?"
                        ),
                        style="color:rgba(255,255,255,0.9)",
                    )
                )
        ...
        ui.nav_panel(
            ...
            ui.row(
                ui.h1(
                    ui.span(
                        HTML("Is there racial bias in the American justice system?"),
                        style="color:rgba(255,255,255,0.9)",
                    )
                )
            ),
        ...
        ui.nav_panel(
            ...
            ui.row(
                ui.h1(
                    ui.span(
                        HTML("Fossil fuels are not the cheapest sources for energy."),
                        style="color:rgba(255,255,255,0.9)",
                    )
                )
            ),
...
def server(input, output, session):
    ...
    def plot_economy_timeseries_income():
        ...
        usa = shares_wid.filter(pl.col("country") == "usa").filter(
            pl.col("year") >= 1880
        )
    ...
    def plot_economy_barchart_income_countries():
        ...
        income_latest = shares_wid.filter(
            pl.col("year") == shares_wid["year"].max()
        ).sort(
            [
                income_level,
            ],
    ...
    def plot_economy_timeseries_income_policies():
        ...
        auth = shares_wid.filter(pl.col("country").is_in(["russia", "china"]))
    ...
    def plot_economy_f150():
        ...
        data = f150.join(
            ...
            on=["year"],
            how="inner",
        ).with_columns(price_ratio=pl.col("price") / pl.col(income_level))
    ...
    def plot_economy_house_purchase_cost_as_percent_of_income():
        ...
        data = house_purchase_cost_as_percent_of_income.join(
            ...
            house_purchase_cost_as_percent_of_income=pl.col("cost")
            / pl.col(income_level)
        )
        ...
        yaxis_min, yaxis_max = get_yaxis_range(
            y_data=data["house_purchase_cost_as_percent_of_income"]
        )
        fig.update_layout(
            title=dict(
    ...
    def plot_american_dream_kids():
        ...
        data = mobility_international.filter(pl.col(col).is_not_nan()).filter(
            pl.col("country").is_in(config["countries"])
        )
        last = data.join(
            other=(data.group_by("country").agg(pl.max("year"))),
            on=["country", "year"],
            ...
            data=(
                [
                    go.Scatter(
                        ...
                        line=(
                            dict(color=color_light_dark[dark_mode], width=3)
        ...
        fig.update_layout(
            title=dict(
                text="<b>Upward Mobility</b><br><sup>Kids doing better than parents</sup>",
            ),
            title_x=0.5,
    ...
    def plot_american_dream_mobility_international():
            ...
            data=(
                ...
                + [
                    ...
                    for country in config[
                        "countries"
                    ]  # ["United States", "Norway", "Sweden", "France", "Australia"]
                ]
            )
        )
        ...
        fig.update_layout(
            title=dict(
                text="<b>Upward Mobility Breakdown</b><br><sup>Fixed income growth, impact of Top 1% taking more</sup>",
            ),
            title_x=0.5,
            yaxis_title="% of 30 year olds that earn more than parents",
            xaxis_title=f"{sources['american_dream']}",
    ...
    def plot_black_upward_mobility():
        ...
        fig = plot_county_heatmap(
            ...
            race="black",
            metric=metric,
            title="Black Male Upward Mobility<br>(parents in bottom 25th of income)",
    ...
    def plot_white_upward_mobility():
        ...
        fig = plot_county_heatmap(
            ...
            race="white",
            metric=metric,
            title="White Male Upward Mobility<br>(parents in bottom 25th of income)",
            colorscale=config["colorscale"][metric],
            dark_mode=input.dark_mode(),
        )
        return fig
    ...
    def plot_black_jail():
        ...
        fig = plot_county_heatmap(
            ...
            race="black",
            metric=metric,
            title="Black Male Jail Rate<br>(parents in bottom 25th of income)",
            colorscale=config["colorscale"][metric],
            dark_mode=input.dark_mode(),
        )
        return fig
    ...
    def plot_white_jail():
        ...
        fig = plot_county_heatmap(
            ...
            race="white",
            metric=metric,
            title="White Male Jail Rate<br>(parents in bottom 25th of income)",
            colorscale=config["colorscale"][metric],
            dark_mode=input.dark_mode(),
        )
        return fig
    ...
    def plot_electricity_cost():
        ...
        for technology in electricity_cost.select(["Technology"]).to_numpy().flatten():
            row = electricity_cost.filter(pl.col("Technology") == technology)
            category = row["Category"].to_numpy()[0]
            counts[category] += 1
            fig.add_trace(
                go.Scatter(
                    x=[
                        row["LCOE_Low_USD_MWh"].to_numpy().flatten()[0],
                        row["LCOE_High_USD_MWh"].to_numpy().flatten()[0],
                    ],
                    y=[
                        row["Technology"].to_numpy().flatten()[0],
                        row["Technology"].to_numpy().flatten()[0],
                    ],
                    mode="lines+markers",

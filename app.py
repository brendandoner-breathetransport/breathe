...
upward_mobility = read_data(file_name="Table_5_national_estimates_by_cohort_primary_outcomes").with_columns((pl.col("cohort") + 30).alias("cohort"))
...
app_ui = ui.page_fillable(
    ui.page_navbar(
        ...
        ui.nav_panel(
            ...
            ui.row(
                ui.layout_columns(
                    ui.card(output_widget("plot_fairness_upward_mobility")),
                    ui.card(output_widget("plot_fairness_income_comparison")),
                    ui.card(output_widget("plot_fairness_rank_mobility")),
                    col_widths=(4, 4, 4),
                )
            ),
            ui.row(
                ui.layout_columns(
                    ui.card(output_widget("plot_fairness_employment_rate")),
                    col_widths=(12,),
                )
            ),
...
def server(input, output, session):
    ...
    @output
    @render_widget
    def plot_fairness_upward_mobility():
        ...
        for race in races:
            fig.add_trace(
                go.Scatter(
                    x=df["cohort"],
                    y=df[f"kfr_{race}_pooled_p1"],
                    name=race.capitalize()
                )
            )

        fig.update_layout(
            title="Upward Mobility Rate (Bottom 20%) by Race",
    ...
    @output
    @render_widget
    def plot_fairness_income_comparison():
        import plotly.graph_objects as go
        import pandas as pd

        df = pd.read_csv("data/Table_5_national_estimates_by_cohort_primary_outcomes.csv")
        races = ["white", "black", "hisp", "asian", "aian"]

        fig = go.Figure()
        for race in races:
            fig.add_trace(go.Scatter(x=df["cohort"], y=df[f"kfi_{race}_pooled_p100"],
                                    name=f"{race.capitalize()} - Child"))
            fig.add_trace(go.Scatter(x=df["cohort"], y=df[f"kii_{race}_pooled_p100"],
                                    name=f"{race.capitalize()} - Parent", line=dict(dash="dash")))

        fig.update_layout(
            title="Child vs Parent Income Over Time",
            xaxis_title="Birth Cohort",
            yaxis_title="Income ($)",
            template="plotly_white",
        )
    ...
    @output
    @render_widget
    def plot_fairness_rank_mobility():
        import plotly.graph_objects as go
        import pandas as pd
        ...
        fig = go.Figure()
        for race in races:
            fig.add_trace(go.Scatter(x=df["cohort"], y=df[f"kir_{race}_pooled_p1"],
                                    name=race.capitalize()))

        fig.update_layout(
            title="Income Rank Mobility (Bottom 20%) by Race",
            xaxis_title="Birth Cohort",
            yaxis_title="Income Rank (0–100)",
            template="plotly_white",
        )
        return fig
